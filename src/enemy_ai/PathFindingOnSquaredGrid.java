...
public class PathFindingOnSquaredGrid {
     ...
     */
    public void generateHValue(boolean matrix[][], int Ai, int Aj, int Bi, int Bj, int v, int d, boolean additionalPath, int h, int[][] movMatrix) {
        ...
        generatePath(cell, Ai, Aj, Bi, Bj, v, d, additionalPath, movMatrix);
    }
    ...
    public ArrayList<Node> findPath(Tile startTile, Tile endTile) {
        ...
        pathList.clear();
        boolean isFlier = false;
        if (startTile.carrier != null) isFlier = startTile.carrier.isFlier;
        
        //Generating a new Boolean Matrix according to the input values of n and p (Length, Percolation value)
        boolean[][] randomlyGenMatrix = finder.boolMatrix(isFlier); // <- where we put in our grid
        int[][] movMatrix = finder.movMatrix(isFlier);
        ...
        generateHValue(randomlyGenMatrix, Ai, Aj, Bi, Bj, 1, 2, false, 3, movMatrix);
     ...
     * @param additionalPath Boolean to decide whether to calculate the cost of through the diagonal path
     */
    public void generatePath(Node hValue[][], int Ai, int Aj, int Bi, int Bj, int v, int d, boolean additionalPath, int[][] movMatrix) {

        //Creation of a PriorityQueue and the declaration of the Comparator
		PriorityQueue<Node> openList = new PriorityQueue<>(11, new Comparator<Object>() {
            @Override
        ...
        while (true) {
            ...
            node.setMoveCost(movMatrix[node.x][node.y]);
            ...
            try {
                if (cell[node.x][node.y - 1].hValue != -1
                	...
                	movCost = movMatrix[node.x][node.y-1];
                    ...
                    if (cell[node.x][node.y - 1].fValue > cost || !openList.contains(cell[node.x][node.y - 1]))
                        cell[node.x][node.y - 1].fValue = cost + movCost;
            ...
            try {
                if (cell[node.x + 1][node.y].hValue != -1
                	...
                	movCost = movMatrix[node.x+1][node.y];
                    double tCost = node.fValue + v;
                    ...
                    if (cell[node.x + 1][node.y].fValue > cost || !openList.contains(cell[node.x + 1][node.y]))
                        cell[node.x + 1][node.y].fValue = cost + movCost;
